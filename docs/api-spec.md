...
| --- | --- | --- | --- |
| **Core API** | Spring Boot + Spring Security(JWT) | 회원가입/로그인(아이디/비밀번호), 서재(소설/챕터), 용어집, API 키·테마 설정, **AI API로의 내부 프록시** | `http://localhost:8080` |
...
일반적인 아이디/비밀번호 로그인을 사용한다(OAuth2는 향후 확장 항목으로 보류, 10장 Open Items 참고). 회원가입은 아이디/비밀번호/비밀번호 확인만 입력받는 별도 절차다.

**회원가입 시퀀스**

1. (선택) 프론트엔드가 입력 중인 아이디에 대해 `GET /api/v1/auth/check-username`으로 실시간 중복 확인.
2. `POST /api/v1/auth/signup`으로 아이디/비밀번호/비밀번호 확인을 전송, 성공 시 로그인과 동일하게 Access Token + Refresh Token(쿠키)이 즉시 발급된다(가입과 동시에 로그인 처리).

**로그인 시퀀스**

1. `POST /api/v1/auth/login`으로 아이디/비밀번호 전송.
2. 성공 시 Access Token을 응답 body로, Refresh Token을 `Set-Cookie`로 받는다.
3. 이후 요청은 `Authorization: Bearer {accessToken}` 헤더 사용, 만료 시 `POST /api/v1/auth/refresh` 로 재발급(재발급 시 Refresh Token도 함께 회전됨).

### 1.2 `GET /api/v1/auth/check-username`

**설명**: 아이디 중복 여부를 조회한다(회원가입 폼의 실시간 중복확인용, 인증 불필요).
...
{
  "status": 200,
  "data": { "username": "hero123", "available": true },
  "message": "조회 성공",
  "error": null
}
```

### 1.3 `POST /api/v1/auth/signup`

**설명**: 아이디/비밀번호로 회원가입한다. 성공 시 로그인과 동일한 형태로 Access/Refresh Token을 즉시 발급한다(가입 = 자동 로그인).

**Request Body**

| 필드명 | 타입 | 설명 | 필수 |
| --- | --- | --- | --- |
| username | String | 영문/숫자/밑줄 4~20자 | ✅ |
...
```json
{
  ...
  "data": {
    ...
    "user": { "id": "5e1c...", "username": "hero123", "createdAt": "2026-07-04T09:00:00Z" }
  },
...
| `PASSWORD_MISMATCH` | 400 | `password`와 `passwordConfirm`이 다름 |
| `VALIDATION_ERROR` | 400 | 아이디/비밀번호 형식이 규칙에 맞지 않음 |
...
**📥 Response**: 1.3과 동일한 형태(`status: 200`, `message: "로그인에 성공했습니다."`), 동일하게 `Set-Cookie`로 Refresh Token 포함.

**🔥 ERROR**

| 에러 코드 | HTTP 상태 | 발생 조건 |
| --- | --- | --- |
| `INVALID_CREDENTIALS` | 401 | 아이디가 없거나 비밀번호가 일치하지 않음(탈퇴 계정 포함) |

### 1.5 `POST /api/v1/auth/refresh`

**설명**: 쿠키의 Refresh Token으로 새 Access Token을 발급한다. Body 없음, 쿠키만 사용. **쿠키만으로 인증되는 요청**이라 `X-Requested-With: XMLHttpRequest` 헤더를 반드시 포함해야 한다(CSRF 방어, 아래 참고).
...
| 에러 코드 | HTTP 상태 | 발생 조건 |
| --- | --- | --- |
| `CSRF_HEADER_MISSING` | 403 | `X-Requested-With` 헤더가 없거나 값이 다름 |
| `REFRESH_TOKEN_INVALID` | 401 | 쿠키가 없거나, DB에 없거나(로그아웃됨), 만료됨 |

**CSRF 방어 노트**: 이 API는 Bearer 토큰이 아니라 쿠키(브라우저가 자동으로 첨부)만으로 동작하는 유일한 인증 필요 엔드포인트라 CSRF 표적이 될 수 있다. Refresh Token 쿠키의 `SameSite=Lax`가 cross-site POST에는 쿠키 자체를 안 실어 보내므로 1차 방어가 되지만, `X-Requested-With` 커스텀 헤더 요구를 한 겹 더 둔다 — 단순 HTML 폼 기반 CSRF는 커스텀 헤더를 붙일 수 없고, 다른 오리진의 fetch/XHR이 이 헤더를 붙이면 CORS preflight가 발생해 허용되지 않은 오리진의 요청은 브라우저가 막는다. `POST /api/v1/auth/logout`도 동일하게 이 헤더를 요구한다(아래 1.6).

### 1.6 `POST /api/v1/auth/logout`

**설명**: 쿠키로 전달된 Refresh Token을 DB에서 무효화하고 쿠키를 삭제한다. Refresh Token 쿠키 자체가 곧 로그아웃 대상 식별자이므로 별도 `Authorization` 헤더는 요구하지 않는다. 1.5와 동일하게 `X-Requested-With: XMLHttpRequest` 헤더 필수(CSRF 방어).
...
### 1.8 `DELETE /api/v1/users/me`

**설명**: 회원 탈퇴. 사용자 레코드에 `withdrawn_at`을 기록하는 소프트 삭제 처리만 수행한다(인증 필요). `novels`/`chapters`/`api_keys`/`theme_*` 등 소유 데이터의 cascade 정리와 기존 Refresh Token 즉시 무효화는 아직 구현되지 않았다 — 10장 Open Items 참고.
...
**설명**: 모델 설정을 저장하고, `apiKeys`가 함께 오면 키 목록(로테이션 순서 포함)도 전체 교체한다. 키는 서버에서 AES-256-GCM으로 암호화되어 `api_keys.encrypted_key`에 저장된다. `apiKeys`를 생략하면 키 목록은 건드리지 않고 `model`/`thinkingBudget`만 갱신한다 — `GET`이 마스킹된 값만 돌려주기 때문에 프론트가 기존 키를 다시 입력하지 않고는 전체 교체를 할 수 없어서다(키 하나만 추가하려면 4.4 참고).

**Request Body**

| 필드명 | 타입 | 설명 | 필수 |
| --- | --- | --- | --- |
| apiKeys | String[] | 줄 단위로 입력된 키 목록, 배열 순서 = 로테이션 순서. 생략 시 기존 키 유지 | ❌ |
...
### 4.4 `POST /api/v1/settings/api/keys`

**설명**: 키 하나만 기존 목록 끝에 추가한다(`key_order`는 현재 개수). `GET`이 마스킹된 값만 돌려주기 때문에 기존 키를 다시 입력하지 않고는 4.2(PUT, 전체 교체)로 "키 하나만 추가"를 할 수 없다 — 프론트의 "새 키 추가" 입력은 이 엔드포인트를 쓴다.

**Request Body**

| 필드명 | 타입 | 설명 | 필수 |
| --- | --- | --- | --- |
| apiKey | String | 추가할 키 1개 | ✅ |

**📥 Response**: `201 Created`, `data`는 4.1과 동일한 형태(갱신된 전체 키 목록).

**🔥 ERROR**: 4.2와 동일(`INVALID_API_KEY_FORMAT`).

### 4.5 `POST /api/v1/settings/api/keys/bulk`

**설명**: 여러 키를 한 번에 붙여넣기(줄바꿈 구분)로 등록할 때 쓰는 벌크 추가 엔드포인트. 4.4를 키 개수만큼 반복 호출하는 대신 한 번의 요청으로 처리하고, 입력된 배열 순서 그대로 `key_order`를 이어붙인다(`nextOrder = 현재 키 개수`, 이후 순차 증가) — 여러 번의 왕복이나 병렬 호출로 인한 순서 꼬임을 피하기 위함.

**Request Body**

| 필드명 | 타입 | 설명 | 필수 |
| --- | --- | --- | --- |
| apiKeys | String[] | 추가할 키 목록, 배열 순서 = 로테이션 순서로 이어붙임 | ✅ (최소 1개) |
...
| 에러 코드 | HTTP 상태 | 발생 조건 |
| --- | --- | --- |
| `INVALID_API_KEY_FORMAT` | 400 | 키 중 하나라도 ASCII 출력 가능 문자 이외 문자가 포함됨 |
| `VALIDATION_ERROR` | 400 | `apiKeys`가 비어 있거나 빈 문자열을 포함 |
...
- 이 API는 6.2가 내부적으로 호출한다. 프론트가 직접 호출할 일은 거의 없다(이전/다음 챕터 이동처럼 novelId를 이미 아는 경우는 3.2를 직접 쓴다).

### 6.2 `POST /api/v1/read`

**설명**: 메인 입력창(URL 또는 붙여넣은 텍스트)의 실제 진입점. "먼저 2.2로 서재에 소설을 등록하고 그 다음 3.2로 챕터를 등록"하는 2단계를 사용자가 직접 거치지 않도록, 소설/챕터 조회부터 필요 시 크롤링 및 서재 자동 생성까지 한 번에 처리한다. **기본 동작은 캐시 우선**이다 — 이미 저장된 챕터면 크롤링도 번역도 다시 하지 않고 DB에 있는 그대로 돌려준다. 번역 스트림(7.1) 호출 여부는 프론트가 응답의 `translatedText`를 보고 판단한다: 비어 있으면(최초 진입이거나 이전 시도가 중간에 끊겨 저장 전에 실패한 경우) 7.1을 호출해 번역하고, 값이 있으면 그대로 화면에 표시하고 7.1을 호출하지 않는다. 사용자가 명시적으로 "다시 번역" 버튼을 눌렀을 때만 `forceRecrawl=true`로 이 API를 다시 호출한다 — 이 경우에만 원문을 재크롤링하고, 이어서 프론트가 무조건 7.1을 호출해 재번역한다. (인증 필요, `Authorization: Bearer`)

**Request Body**: `sourceUrl`(URL 입력 시) 또는 `pastedText`(붙여넣기 시) 중 정확히 하나만 채운다.

| 필드명 | 타입 | 설명 | 필수 |
| --- | --- | --- | --- |
| sourceUrl | String | 회차 URL. `pastedText`와 동시 사용 불가 | ❌ |
| pastedText | String | 직접 붙여넣은 원문. `sourceUrl`과 동시 사용 불가 | ❌ |
| forceRecrawl | Boolean | "다시 번역" 버튼으로 명시적 재요청 시 `true`. 기본 `false` | ❌ |

**동작**

- `sourceUrl`이 주어진 경우:
  1. 로그인 사용자의 챕터 중 동일 `sourceUrl`을 가진 챕터가 이미 있고 `forceRecrawl`이 아니면, 그 챕터를 그대로 반환한다(캐시 히트 — 크롤링도 번역도 하지 않음).
  2. 챕터가 없거나(최초 진입) `forceRecrawl=true`이면 9.1 크롤링을 호출해 `title`/`content`/`prevUrl`/`nextUrl`/`siteName`을 새로 얻는다.
     - 챕터가 없던 경우: 크롤링 `title`로 새 `Novel`을 자동 생성(`sourceUrl`=요청 URL, `siteName`=크롤링 결과)하고, 그 아래 새 `Chapter`(원문만, `translatedText` 빈 문자열)를 생성한다.
     - `forceRecrawl=true`로 기존 챕터가 있던 경우: 새 `Novel`을 만들지 않고 기존 `Chapter`의 `original_text`/`prevUrl`/`nextUrl`만 갱신한다(`translatedText`는 그대로 둔다 — 어차피 프론트가 이어서 7.1을 호출해 덮어쓴다).
- `pastedText`가 주어진 경우: 항상 새 `Novel`(`title`="새 도서", `sourceUrl`=`pasted:{uuid}`, `siteName`="pasted")과 그 아래 `pastedText`를 원문으로 하는 새 `Chapter`를 생성한다. 붙여넣기는 매번 새 항목으로 취급한다(내용이 같아도 재사용 판단 안 함, `forceRecrawl` 의미 없음).
...
{
  "status": 200,
  "data": { "novelId": "n_01", "chapterId": "c_012", "translatedText": "" },
  "message": "조회/등록 성공",
  "error": null
}
```
...
| 에러 코드 | HTTP 상태 | 발생 조건 |
| --- | --- | --- |
| `VALIDATION_ERROR` | 400 | `sourceUrl`/`pastedText`가 둘 다 비었거나 둘 다 채워짐 |
| `UNSUPPORTED_SITE` / `CRAWL_TARGET_ERROR` / `PARSE_FAILED` / `AI_API_UNAVAILABLE` | 6.1과 동일 | `sourceUrl` 크롤링 과정에서 발생 |
...
| 구분 | 대상 API |
| --- | --- |
| 인증 불필요 | `GET /api/v1/auth/check-username`, `POST /api/v1/auth/signup`, `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh`(쿠키 사용), `POST /api/v1/auth/logout`(쿠키 사용) |
| 인증 필요(Bearer) | 그 외 Core API 전체 (1.7~7장) |
...
- [x] Spring Boot 프로젝트 초기화 (Spring Security, Spring Data JPA, Flyway 마이그레이션)
- [x] FastAPI 프로젝트 초기화 (기존 `src/crawl/parser/*.ts` 파서 로직을 Python(BeautifulSoup 등)으로 이식)
- [x] 아이디/비밀번호 회원가입·로그인(JWT) 구현
- [ ] OAuth2(Google/GitHub) 로그인 — 향후 확장 항목으로 보류. 붙일 경우 `users` 테이블에 `oauth_provider`/`oauth_id` 재도입 검토 필요(현재는 `username`/`password_hash`만 존재)
- [ ] 회원 탈퇴(`DELETE /api/v1/users/me`) 시 소유 데이터(`novels`/`chapters`/`api_keys`/`theme_*`) cascade 정리 및 기존 Refresh Token 즉시 무효화 — 현재는 소프트 삭제(`withdrawn_at`)만 수행
