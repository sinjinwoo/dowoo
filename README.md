...
- **Gemini API 연동**: API 키 여러 개를 줄 단위로(또는 한 번에 여러 개) 등록하면 요청마다 무작위로 순환 사용(한도 분산), 번역 모델 선택(자동 모드는 `Gemini 3.1 Flash Lite → Gemini 3 Flash → Gemini 2.5 Flash → Gemini 3.5 Flash` 순으로 무료 모델을 시도, Pro 등 유료 모델은 직접 선택 시에만 사용). 원문은 1만자 단위로 잘라 순차 번역하며, 청크가 성공할 때마다 다음 키로 넘어가 RPM 한도가 특정 키에 몰리지 않게 분산
